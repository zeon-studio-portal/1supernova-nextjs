...
export const getSinglePage = (folder) => {
  ...
  const sanitizeFiles = filesPath.filter(
    (file) => file.includes('.mdx') || file.includes('.md')
  );
  const filterSingleFiles = sanitizeFiles.filter((file) =>
    file.match(/^(?!_)/)
  );
  const singlePages = filterSingleFiles.map((filename) => {
    const slug = filename.includes('.mdx') ? filename.replace('.mdx', '') : filename.replace('.md', '');
  ...
  const publishedPages = singlePages.filter(
    (page) => !page.frontmatter.draft && !page.frontmatter.layout && page
  );
